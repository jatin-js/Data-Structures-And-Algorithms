...
using namespace std;
int lazy[1000];
int rangemax(vector<int> &tree, vector<int> &lazy, int index, int s, int e, int sq, int eq){
    ...
    int mid = (s+e)/2;
    return max(rangemax(tree, lazy, 2*index, s, mid, sq, eq), rangemax(tree, lazy, 2*index+1, mid+1, e, sq, eq));
}
...
int main() {
    ...
    cout<<rangemax(tree, lazy, 1, 0, n-1, 1, 3)<<' ';
